...
internal class MainStartRuleUnacceptedInterceptor
@Inject constructor() : Interceptor {
    ...
    private fun getRulesStatusResponse(chain: Interceptor.Chain): Response {
        return Response.Builder()
                ...
                .request(chain.request())
                .protocol(Protocol.HTTP_1_0)
                .body(ResponseBody.create("application/json".toMediaTypeOrNull(), ruleStatusUnaccepted()))
                .addHeader("content-type", "application/json")
                .build()
    }

    private fun ruleStatusUnaccepted(): String {
        return """{
                "result": [
                    {
                        "rulesId": 7,
